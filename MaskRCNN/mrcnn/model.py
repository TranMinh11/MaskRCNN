...
class PyramidROIAlign(KL.Layer):
    ...
    def call(self, inputs):
        ...
        pooled = np.reshape(pooled, shape)
        return pooled
...
def overlaps_graph(boxes1, boxes2):
    ...
    b1 = np.reshape(tf.tile(tf.expand_dims(boxes1, 1),
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    rpn_class_logits = KL.Lambda(
        lambda t: np.reshape(t, [tf.shape(t)[0], -1, 2]))(x)
    ...
    rpn_bbox = KL.Lambda(lambda t: np.reshape(t, [np.shape(t)[0], -1, 4]))(x)
